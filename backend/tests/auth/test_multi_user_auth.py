...
class MultiUserIsolationTests(unittest.TestCase):
    ...
    def test_profiles_custom_scenarios_sessions_and_reset_are_isolated(self) -> None:
        ...
        with bind_user(first_id):
            learning_plan_engine.save_profile(self._profile("A2", "Music"))
            with session_scope() as session:
                ...
                first_job_id = job.id
            self.assertEqual(
                learning_plan_engine.get_generation(first_job_id).job_id,
                first_job_id,
            )
            first_custom = learning_plan_engine.create_roleplay_scenario(
                RoleplayScenarioCreate(
                    category="Music",
            ...
            first_started = learning_plan_engine.start_roleplay_session(
                RoleplaySessionStart(
        ...
        with bind_user(second_id):
            learning_plan_engine.save_profile(self._profile("B2", "History"))
            second_started = learning_plan_engine.start_roleplay_session(
                RoleplaySessionStart(
                    client_session_id=self.client_session_id,
                    scenario_id="airport_check_in",
                )
            ...
            self.assertEqual(learning_plan_engine.get_profile().interests, ["History"])
            self.assertNotIn(
                first_custom.id,
                {item.id for item in learning_plan_engine.list_roleplay_scenarios()},
            )
            self.assertEqual(
                learning_plan_engine.get_roleplay_transcript(
                    self.client_session_id
                ).turns,
                [],
            )
            with self.assertRaises(PlpNotFoundError):
                learning_plan_engine.get_generation(first_job_id)
            learning_plan_engine.record_roleplay_turn(
                self.client_session_id,
                RoleplayTurnInput(
            ...
            self.assertEqual(len(learning_plan_engine.list_roleplay_sessions()), 1)
        ...
        with bind_user(first_id):
            ...
            learning_plan_engine.reset_local_learner()
            with self.assertRaises(PlpNotFoundError):
                learning_plan_engine.get_profile()
            self.assertEqual(learning_plan_engine.list_roleplay_sessions(), [])
