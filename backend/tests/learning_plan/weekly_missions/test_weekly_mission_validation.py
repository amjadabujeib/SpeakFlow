...
class WeeklyMissionValidationTests(WeeklyMissionCompilerTestBase):
    ...
    def test_semantic_failure_still_spends_only_one_provider_call(self):
        ...
        self.assertEqual(raised.exception.failure_kind, "content_validation")

    def test_unanchored_paraphrase_falls_back_to_safe_source_definition(self):
        def concept(word, definition, suffix):
            return RetrievedConcept(
                id=f"cefr_j_profiles_2020:vocabulary:{suffix}",
        ...
        prepared = prepare_week(
            lessons=self.lessons,
            chunks=self.chunks,
            lexical_palette=[unsafe, database, network],
            support_language="Arabic",
        )
        targets = [
        ...
        self.assertEqual(targets, ["database", "network"])

        draft = _valid_draft(prepared)
        contexts = [item.model_dump() for item in draft.context_realizations]
        request_id = next(
            request.request_id
            for request in prepared.context_requests.values()
            if request.prototype_target is database
        )
        next(item for item in contexts if item["request_id"] == request_id)[
            "learner_definition"
        ] = "a structured collection of data"
        compiled = _compile(
            prepared,
            _replace_draft(draft, context_realizations=contexts),
        )
        card = next(
            activity
            ...
            and activity["data"].get("concept_id") == database.id
        )
        self.assertEqual(card["data"]["definition"], database.definition)
        self.assertEqual(card["data"]["definition_origin"], "source")
    ...
    def test_checkpoint_does_not_reuse_a_teaching_prompt_answer_fingerprint(self):
        ...
        for lesson in self.lessons[:-1]:
            teaching.update(
                _scored_fingerprints(self.compiled[lesson["lesson_key"]][0]["content"])
            )
    ...
    def test_rejects_missing_request_ids(self):
        ...
        with self.assertRaisesRegex(
            ValueError, r"context realizations mismatch; missing="
        ):
    ...
    def test_checkpoint_uses_a_compiler_owned_literal_fact_prompt(self):
        ...
        request = self.prepared.stimulus_requests[
            values[checkpoint_index]["request_id"]
        ]
    ...
    def test_rejects_stimulus_without_its_canonical_answer(self):
        values = [item.model_dump() for item in self.draft.stimulus_realizations]
        values[0]["text"] = "Only unrelated evidence appears in this input."
    ...
    def test_stated_writer_distractor_uses_reviewed_alternatives(self):
        ...
        option_texts = {
            item["text"] for item in activity["data"]["question"]["options"]
        }
    ...
    def test_structured_dialogue_turns_are_counted_separately(self):
        ...
        values[0]["sentences"] = [
            ...
            "Speaker two: This separate turn confirms the practice schedule",
    ...
    def test_fixed_three_part_provider_stimulus_compiles(self):
        ...
        values[0]["text"] = ""
        values[0]["opening"] = (
            "The community team compares two real tools for its project."
        )
        values[0]["evidence"] = (
    ...
    def test_short_input_is_rejected_instead_of_padded_with_boilerplate(self):
        values = [item.model_dump() for item in self.draft.stimulus_realizations]
        request = self.prepared.stimulus_requests[values[0]["request_id"]]
        values[0]["text"] = f"{request.canonical_answer}. Team checks. They agree."
    ...
    def test_rejects_learner_instruction_inside_listening_input(self):
        ...
        request = next(
            ...
            for item in values
            if self.prepared.stimulus_requests[item["request_id"]].mode == "listening"
        )
        target = next(
    ...
    def test_preserves_reviewed_anchor_when_context_exceeds_cefr_limit(self):
        ...
        values[0]["sentence"] = f"{request.required_phrase} " + " ".join(
            f"extra{index}" for index in range(maximum + 1)
        )
    ...
    def test_preserves_reviewed_anchor_when_required_phrase_is_missing(self):
        values = [item.model_dump() for item in self.draft.context_realizations]
        request = self.prepared.context_requests[values[0]["request_id"]]
        values[0]["sentence"] = "The community team discusses a different topic."
    ...
    def test_accepts_context_at_the_exact_hard_cefr_sentence_limit(self):
        ...
        values[0]["sentence"] = f"{request.required_phrase} " + " ".join(
            f"boundary{index}" for index in range(maximum - 1)
        )
