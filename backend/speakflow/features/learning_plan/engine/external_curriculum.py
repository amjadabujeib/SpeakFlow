...
from .interest_vocabulary import source_interest_tags
from .learner_glosses import reviewed_wordnet_synset
from .lexical_policy import normalized_part_of_speech, supports_lexical_prototype

DEFAULT_CEFR_J_ROOT = Path(DATA_ROOT / "curriculum_sources" / "cefr_j" / "raw")
DEFAULT_WORDS_CEFR_ROOT = Path(DATA_ROOT / "curriculum_sources" / "words_cefr" / "raw")
...
def concept_from_vocabulary_row(row: dict[str, str]) -> ExternalConceptRecord:
    ...
    return ExternalConceptRecord(
        ...
        title=headword,
        description=None,
        topic_tags=source_interest_tags(topic_text, title=headword),
        attributes={
...
def concept_from_grammar_row(
    ...
    external_key = hashlib.sha256(f"{source_id}\x1f{shorthand}".encode()).hexdigest()[
        :24
    ]
    return ExternalConceptRecord(
...
def concept_from_evp_row(row: dict[str, str]) -> ExternalConceptRecord:
    ...
    return ExternalConceptRecord(
        ...
        topic_tags=source_interest_tags(topic, title=headword),
        attributes={
            "part_of_speech": part_of_speech or None,
...
def read_cefr_j(root: Path = DEFAULT_CEFR_J_ROOT) -> list[ExternalConceptRecord]:
    ...
    with (root / "cefrj-vocabulary-profile-1.5.csv").open(
        ...
        records.extend(
            concept_from_vocabulary_row(row) for row in csv.DictReader(source)
        )
...
def read_word_frequencies(root: Path = DEFAULT_WORDS_CEFR_ROOT) -> dict[str, int]:
    words: dict[str, str] = {}
    with (root / "csv" / "words.csv").open(encoding="utf-8-sig", newline="") as source:
        for row in csv.DictReader(source):
...
def add_lexical_reference_evidence(
    ...
    for record in records:
        if record.concept_type != "vocabulary":
            enriched.append(record)
            continue
        part_of_speech = record.attributes.get("part_of_speech")
        if not supports_lexical_prototype(part_of_speech):
            enriched.append(record)
            continue
        pos = _WORDNET_POS[normalized_part_of_speech(part_of_speech)]
...
def _select_wordnet_synset(
    ...
    CEFR-J assigns levels to word/POS rows, not WordNet senses. Only explicit,
    reviewed exceptions may override WordNet's first, corpus-frequency sense.
    """
    override = reviewed_wordnet_synset(
        record.title,
        record.attributes.get("part_of_speech"),
    )
    if override:
        selected = next(
