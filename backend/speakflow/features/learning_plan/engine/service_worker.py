...
class PlpWorkerMixin:
    def _worker_loop(self) -> None:
        while not self._stop_event.is_set():
            if not self._reviewed_curriculum_is_current():
    ...
    def _claim_next_job(self) -> uuid.UUID | None:
        try:
            with session_scope() as session:
                ...
                if job is None:
                    # A completion request can commit while the process is
                    # stopping, or an older release may have missed its one-shot
                    # queue transition. Reconcile idle jobs so eligible content
                    # is eventually generated without another learner attempt.
                    idle_jobs = session.scalars(
                        select(GenerationJob)
                        .where(GenerationJob.status == "idle")
                        .order_by(GenerationJob.updated_at)
                        .with_for_update(skip_locked=True)
                    ).all()
                    job = next(
                        (
                            idle_job
                            for idle_job in idle_jobs
                            if _eligible_pending_lesson_ids(
                                session,
                                idle_job.revision_id,
                            )
                        ),
                        None,
                    )
    ...
    def _process_job(self, job_id: uuid.UUID) -> None:
        with session_scope() as session:
            ...
            is_weekly_mission = bool(
                first_eligible
                and first_eligible.specification.get("architecture") == PLP_ARCHITECTURE
            )
