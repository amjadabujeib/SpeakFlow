...
from .weekly_mission_models import _WORD, StimulusRealization, WeeklyScenarioDraft
...
def _correct_option_text(question: dict) -> str:
    ...
    try:
        return next(
            item["text"] for item in question["options"] if item["id"] == answer_id
        )
...
def _scored_fingerprints(content: dict) -> set[str]:
    ...
    for activity in content["activities"]:
        ...
        result.add(
            hashlib.sha256(
                f"{_normalize(prompt)}\x1f{_normalize(answer)}".encode()
            ).hexdigest()
        )
    return result
