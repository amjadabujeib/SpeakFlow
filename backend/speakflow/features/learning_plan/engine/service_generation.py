...
def _existing_generation_disposition(status: str, reason: str) -> str:
    """Choose whether a nonterminal job is reused, blocks, or is superseded."""

    if reason == "manual_regeneration":
        # An idle JIT job represents the currently active roadmap waiting for
        # learner prerequisites. Reusing it would make the settings action a
        # silent no-op. Keep in-flight provider work idempotent, but replace an
        # idle roadmap with a genuinely new revision.
        return "supersede" if status == "idle" else "reuse"
    if not reason.startswith("adaptation:"):
        return "reuse"
    # Idle JIT jobs are waiting on learner prerequisites, not provider work.
    # An approved adaptation must be able to supersede that future outline.
    return "supersede" if status == "idle" else "block"
...
class PlpGenerationMixin:
    def create_generation(self, *, reason: str = "onboarding") -> GenerationAccepted:
        try:
            with session_scope() as session:
                profile = session.scalar(select(LearnerProfile).where(LearnerProfile.user_id == _user_id()).with_for_update())
                if profile is None:
                ...
                if existing is not None:
                    disposition = _existing_generation_disposition(
                        existing.status,
                        reason,
                    )
                    if disposition == "block":
                        raise PlpConflictError("finish the current plan generation before applying an adaptation")
                    ...
                    superseded_revision = session.get(
                        PlanRevision,
                        existing.revision_id,
                    )
                    if superseded_revision is not None:
                        superseded_revision.status = "superseded"
                    existing.status = "superseded"
                    existing.error = None

                skill_rows = session.scalars(select(Skill).where(Skill.active.is_(True))).all()
                reviewed_chunk_metadata = session.scalars(select(CurriculumChunk.metadata_json).where(CurriculumChunk.review_status == "reviewed")).all()
                source = session.get(CurriculumSource, SOURCE["id"])
                curriculum = assess_curriculum(
                    skill_rows,
                    reviewed_chunk_metadata,
                    reviewed_source_current=reviewed_source_is_current(source.checksum if source is not None else None),
                )
                if not curriculum.supports(profile.cefr_level):
                    raise PlpUnavailableError(CURRICULUM_NOT_READY_MESSAGE)
                planning_skills = [
                    PlanningSkill(
                        id=skill.id,
                        domain=skill.domain,
                ...
                mastered_skill_ids = {
                    ...
                    for row in session.execute(
                        select(
                            ...
                            func.sum(SkillEvidence.score * SkillEvidence.weight) / func.sum(SkillEvidence.weight),
                        )
                        .join(
                            ActivityAttempt,
                    ...
                    if int(row[1] or 0) >= 2 and row[2] is not None and float(row[2]) >= 75
                }
                ...
                if reason.startswith("adaptation:"):
                    ...
                    proposal_plan = session.get(
                        LearningPlan,
                        adaptation_proposal.plan_id,
                    )
                    if proposal_plan is None or proposal_plan.active_revision_id != adaptation_proposal.base_revision_id:
                        raise PlpConflictError("adaptation proposal no longer matches the active plan")
                    base_revision = session.get(
                        PlanRevision,
                        adaptation_proposal.base_revision_id,
                    )
                    priority_skill_ids = [change["skill_id"] for change in adaptation_proposal.changes if change.get("action") == "reinforce" and change.get("skill_id")]
                ...
                if base_revision is not None:
                    old_lessons = session.scalars(select(PlanLesson).where(PlanLesson.revision_id == base_revision.id).order_by(PlanLesson.week_sequence, PlanLesson.lesson_sequence)).all()
                    ...
                    if progress_rows:
                        locked_weeks = max(old_by_id[row.lesson_id].week_sequence for row in progress_rows)
                    if locked_weeks:
                        old_weeks = {week["sequence"]: copy.deepcopy(week) for week in base_revision.outline["weeks"] if week["sequence"] <= locked_weeks}
                        outline["weeks"] = [old_weeks.get(week["sequence"], week) for week in outline["weeks"]]
                        old_lessons_by_key = {lesson.lesson_key: lesson for lesson in old_lessons if lesson.week_sequence <= locked_weeks}
                        old_progress_by_key = {old_by_id[row.lesson_id].lesson_key: row for row in progress_rows if old_by_id[row.lesson_id].week_sequence <= locked_weeks}
                        for attempt in session.scalars(
                            select(ActivityAttempt).where(
                                ActivityAttempt.user_id == _user_id(),
                                ActivityAttempt.lesson_id.in_([item.id for item in old_lessons_by_key.values()]),
                            )
                        ).all():
                            old_attempts_by_key[old_by_id[attempt.lesson_id].lesson_key].append(attempt)
                plan = session.scalar(select(LearningPlan).where(LearningPlan.user_id == _user_id()).order_by(LearningPlan.created_at.desc()))
                if plan is None:
                    plan = LearningPlan(user_id=_user_id())
                    session.add(plan)
                    session.flush()
                max_revision = session.scalar(select(func.max(PlanRevision.revision)).where(PlanRevision.plan_id == plan.id)) or 0
                    ...
                    for unit in week["units"]:
                        for lesson in unit["lessons"]:
                            ...
                            new_lesson = PlanLesson(
                                ...
                                content_status=(old_lesson.content_status if old_lesson else "pending"),
                                content=(copy.deepcopy(old_lesson.content) if old_lesson else None),
                                source_refs=(list(old_lesson.source_refs) if old_lesson else []),
                                generation_attempts=(old_lesson.generation_attempts if old_lesson else 0),
                                generation_error=(old_lesson.generation_error if old_lesson else None),
                            )
    ...
    def get_latest_generation(self) -> GenerationView:
        try:
            with session_scope() as session:
                job = session.scalar(select(GenerationJob).join(PlanRevision, GenerationJob.revision_id == PlanRevision.id).join(LearningPlan, PlanRevision.plan_id == LearningPlan.id).where(LearningPlan.user_id == _user_id()).order_by(GenerationJob.created_at.desc()))
                if job is None:
    ...
    def retry_generation(self, job_id: uuid.UUID) -> GenerationView:
        try:
            with session_scope() as session:
                ...
                source = session.get(CurriculumSource, SOURCE["id"])
                if not reviewed_source_is_current(source.checksum if source is not None else None):
                    raise PlpUnavailableError(CURRICULUM_NOT_READY_MESSAGE)
                ...
                if retry_available_at is not None and utc_now() < retry_available_at:
                    remaining = math.ceil((retry_available_at - utc_now()).total_seconds())
                    raise PlpConflictError(f"Groq is still rate-limited. Retry in {max(1, remaining)} seconds.")
    ...
    def get_active_document(self) -> PlpDocument:
        try:
            with session_scope() as session:
                ...
                job = session.scalar(select(GenerationJob).where(GenerationJob.revision_id == revision.id))
                if job is None:
