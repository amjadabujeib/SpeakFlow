...
def _replace_vocabulary_lesson(
    ...
    *,
    ...
    card_indexes = [
        index
        for index, item in enumerate(template["activities"])
    ...
    if len(distractors) != 2:
        raise ValueError(
            "vocabulary replacement could not build distinct reviewed options"
        )
...
def _choice_data(
    *,
    ...
    values.sort(
        key=lambda pair: hashlib.sha256(f"{seed}\x1f{pair[0]}".encode()).hexdigest()
    )
    options = [
...
def _partition_realizations(
    ...
    dict[str, ContextRealization],
    dict[str, StimulusRealization],
    dict[str, ChoiceRealization],
]:
    ...
    if (
        draft.context_realizations
        or draft.stimulus_realizations
        or draft.choice_realizations
    ):
        raise ValueError(
            "weekly output must not mix flat and legacy realization records"
        )
    ...
    for request_id, item in flat.items():
        if request_id in prepared.context_requests:
            ...
            if (
                any((item.title, item.text, item.prompt, item.explanation))
                or item.distractors
            ):
                raise ValueError(
                    f"{request_id} has non-empty fields unused by a context"
                )
            ...
            if item.sentence:
                raise ValueError(
                    f"{request_id} has a sentence field unused by a stimulus"
                )
            ...
            if any((item.sentence, item.title, item.text)):
                raise ValueError(
                    f"{request_id} has non-empty fields unused by a choice"
                )
