...
DOMAIN_ACTIVITY_TYPES = {
    domain: tuple(blueprint) for domain, blueprint in ACTIVITY_BLUEPRINTS.items()
}
...
class LessonGenerator:
    def __init__(
        ...
        self._groq_pool = GroqClientPool(
    ...
    def request_structured(
        ...
        *,
        ...
        raise GenerationError(f"unsupported PLP generator provider: {self.provider}")
                    ...
                    "content": json.dumps(
                        {
                            ...
                            "validation_error_from_previous_attempt": last_error
                            or None,
                            "output_rule": (
        ...
        *,
        ...
        if not template_records:
            raise GenerationError(
                ...
                "run `python -m speakflow.features.learning_plan.engine.ingest` once to refresh the seed"
            )
        domain = specification["domain"]
        ...
        if domain == "assessment":
            pools = [
                [
                    (candidate, skill_ids)
                    for candidate in assessment_candidates(template)
                ]
    ...
    def _validate_and_assign(
        *,
        ...
        scored = set(SCORED_TYPES)
        if not any(item.type in scored for item in activities):
            raise ValueError(
                "every lesson needs at least one objectively scored activity"
            )
        if any(not item.skill_ids for item in activities):
            raise ValueError(
                "every activity must be bound to at least one assessed skill"
            )
        if domain == "assessment" and any(
            item.type not in scored for item in activities
        ):
            raise ValueError(
                "assessments may contain only objectively scored activities"
            )
