...
class SkillPrerequisite(Base):
    ...
    __table_args__ = (CheckConstraint("skill_id <> prerequisite_skill_id"),)
...
class LearningPlan(Base):
    ...
    __table_args__ = (UniqueConstraint("user_id", name="uq_learning_plans_user_id"),)
...
class LessonProgress(Base):
    ...
    lesson_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("plan_lessons.id", ondelete="CASCADE"),
        primary_key=True,
    )
...
class ActivityAttempt(Base):
    ...
    submission_id: Mapped[str | None] = mapped_column(String(180), nullable=True)
