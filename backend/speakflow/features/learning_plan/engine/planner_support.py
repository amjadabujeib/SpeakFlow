"""Skill ordering and lesson-shell helpers for the mission planner."""

from __future__ import annotations

from .mission_catalog import LessonRole, WeeklyMissionSelection
...
def prioritized_domains(profile: LearnerProfileInput) -> list[str]:
    ...
    tag; this helper remains public for planner and test callers.
    """
...
def _eligible_skills(
    ...
    if len(ids) != len(set(ids)):
        raise ValueError(
            f"reviewed skill catalog has duplicate IDs for {profile.cefr_level}"
        )
    ...
    if invalid_domains:
        raise ValueError(
            f"reviewed skill catalog has unsupported domains: {invalid_domains}"
        )
