...
from .pronunciation_audio import _speech_activity
from .roleplay_evaluation import (
    _arabic_translation_options,
    _finalize_disconnected_roleplay,
    _send_roleplay_tts,
)
from .roleplay_scenario import _roleplay_scenario_draft
from .roleplay_turn import process_roleplay_turn
    ...
    try:
        while True:
            ...
            try:
                ...
                corrected_text = None
                if await asyncio.to_thread(runtime._load_gector_model):
                    async with runtime._chat_inference_lock:
                ...
                safe_error = (
                    ...
                    if isinstance(
                        exc,
                        (ValueError, PlpConflictError, PlpInvalidAttemptError),
                    )
                    else "The roleplay turn could not be processed."
                )
                await websocket.send_text(
                    json.dumps(
                        {
