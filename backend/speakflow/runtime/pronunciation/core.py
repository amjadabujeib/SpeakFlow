...
import numpy as np
from g2p_en import g2p as g2p_module
from g2p_en.expand import normalize_numbers
from nltk import pos_tag


def _construct_homographs_without_leaking_file_handles() -> dict:
    result = {}
    path = Path(g2p_module.dirname) / "homographs.en"
    with path.open(encoding="utf-8") as source:
        for line in source:
            if line.startswith("#"):
                continue
            headword, first, second, first_pos = line.strip().split("|")
            result[headword.lower()] = (
                first.split(),
                second.split(),
                first_pos,
            )
...
# g2p-en 2.1.0 reads this resource without closing it. Keep its model and
# dictionary behavior while replacing only the leaking loader.
g2p_module.construct_homograph_dictionary = (
    _construct_homographs_without_leaking_file_handles
)
G2p = g2p_module.G2p
