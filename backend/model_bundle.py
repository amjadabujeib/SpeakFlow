...
def local_assets() -> list[Asset]:
    ...
    if not curriculum_snapshot.is_file():
        raise FileNotFoundError(
            ...
            "`python -m plp.curriculum_snapshot export` before publishing "
    ...
    return sorted(assets, key=lambda item: item.destination)
...
def upload() -> None:
    ...
    api.upload_file(
        ...
        repo_id=MODEL_REPO,
        repo_type="model",
        commit_message=f"Publish SpeakFlow model bundle {BUNDLE_VERSION}",
    )
    print(
...
def download(revision: str) -> None:
    ...
    for entry in files:
        relative = str(entry["path"])
        _validate_remote_path(relative)
        if _installed(entry):
