...
def _ingest_curriculum() -> None:
    ...
    print(f"\n> {sys.executable} -m speakflow.features.learning_plan.engine.ingest", flush=True)
    subprocess.run(
        [sys.executable, "-m", "speakflow.features.learning_plan.engine.ingest"],
        cwd=BACKEND_ROOT,
        env=environment,
        check=True,
    )
    print(
        f"\n> {sys.executable} -m speakflow.features.learning_plan.engine.curriculum_snapshot import "
        f"--snapshot {snapshot}",
        flush=True,
    )
    subprocess.run(
