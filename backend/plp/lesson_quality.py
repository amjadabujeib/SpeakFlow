...
def validate_lesson_pedagogy(activities: list[Activity], domain: str) -> None:
    ...
    if len(prototype_cards) > 2:
        raise ValueError("a teaching lesson may introduce at most two weekly vocabulary targets")
    ...
    for item in activities:
        ...
        if item.type == "vocabulary_card":
            ...
            context_key = "passage" if item.type == "reading_comprehension" else "transcript"
...
def _validate_choice(data: dict, *, domain: str, context: str | None = None) -> None:
    ...
    if len(options) < 3:
        raise ValueError("a multiple-choice check needs one answer and at least two distractors")
    ...
    if _token_overlap(correct, data["explanation"]) < 0.5:
        raise ValueError("the explanation must explicitly identify the correct response")
...
def _validate_pronunciation(data: dict, seen_items: set[str]) -> None:
    ...
    if not re.search(
        ...
        raise ValueError("pronunciation guidance needs a concrete physical or prosodic cue")
    for practice in items:
