...
def template_search_text(template: dict) -> str:
    ...
    for item in template["activities"]:
        ...
        parts.extend(
            str(data[key])
            for key in (
                "word", "explanation", "sound_label", "prompt", "title",
                "passage", "transcript",
            )
            if data.get(key)
        )
...
def _order(prompt: str, chunks: tuple[str, ...], explanation: str) -> dict:
    ...
    return {
        "prompt": prompt,
        "tokens": [
            {"id": correct_ids[index], "text": chunks[index]}
...
def _vocab(
    *,
    ...
    return {
        ...
        "intro": "Learn two useful items in context, recognize their meaning, then recall one without options.",
        "activities": [
            *[{"type": "vocabulary_card", "data": card} for card in cards],
            {"type": "multiple_choice", "data": check},
...
_CURATED_LESSONS = {
    "A1": {
        "vocabulary": _vocab(
            ...
            cards=(
                _card("usually", "adverb", "/ˈjuːʒuəli/", "on most days or in most situations", "I usually start work at nine.", ["usually go", "usually eat"]),
                _card("nearby", "adverb/adjective", "/ˌnɪrˈbaɪ/", "not far away", "There is a small shop nearby.", ["live nearby", "nearby shop"]),
            ),
        ...
        "pronunciation": _skill_lesson(
            ...
            [
                ...
                {"type": "pronunciation_drill", "data": {"sound_label": "P and B in phrases", "ipa": "/p/", "instructions": "Keep the strong air release on /p/, then switch voicing on for /b/ without adding an extra vowel.", "tips": ["Pause between the contrasting words first.", "Keep both lips fully closed before each sound."], "practice_items": ["paper bag", "Please bring both pens.", "Put the blue book beside the paper."], "native_hint": None}},
...
_CURATED_LESSONS["A2"] = {
    "vocabulary": _vocab(
        ...
        cards=(
            _card("reschedule", "verb", "/ˌriːˈskedʒuːl/", "arrange an event for a different time", "I need to reschedule my appointment.", ["reschedule a meeting", "reschedule for Monday"]),
            _card("receipt", "noun", "/rɪˈsiːt/", "a written or digital record that you paid", "Keep the receipt in case you return the shirt.", ["ask for a receipt", "digital receipt"]),
        ),
    ...
    "pronunciation": _skill_lesson(
        ...
        [
            ...
            {"type": "pronunciation_drill", "data": {"sound_label": "TH in connected phrases", "ipa": "/θ/", "instructions": "Keep the tongue near the teeth and let the air continue as you move smoothly into the next word.", "tips": ["Slow the phrase down before joining the words.", "Check voicing with a finger on your throat."], "practice_items": ["three things", "I thought they were there.", "Their third class is on Thursday."], "native_hint": None}},
...
_CURATED_LESSONS["B1"] = {
    "vocabulary": _vocab(
        ...
        cards=(
            _card("deadline", "noun", "/ˈdedlaɪn/", "the latest time by which something must be completed", "We moved the deadline to Friday.", ["meet a deadline", "miss a deadline"]),
            _card("troubleshoot", "verb", "/ˈtrʌbəlʃuːt/", "identify the cause of a problem and try to solve it", "The technician helped us troubleshoot the connection.", ["troubleshoot an issue", "troubleshooting guide"]),
        ),
    ...
    "reading": _skill_lesson(
        ...
        [
            ...
            _nested("reading_comprehension", title="A quieter office", text="Our team tested a quiet work hour from nine to ten each morning. During the trial, staff completed urgent tasks with fewer interruptions. Because the result was positive, the manager plans to continue the quiet hour next month.", question=_choice("Which result supports continuing the quiet hour?", "Staff completed urgent tasks with fewer interruptions", ("The office opened at nine", "The manager bought new desks"), "The text reports that staff completed urgent tasks with fewer interruptions; this is the supporting result.", correct_position=0)),
            _nested("reading_comprehension", title="Repair before replacement", text="The school considered replacing twenty slow laptops. A technician cleaned the cooling fans and updated the software on five test machines. Those laptops then started much faster, so the school decided to repair the others first.", question=_choice("Why did the school decide to repair the other laptops?", "The five test laptops became faster", ("Twenty new laptops arrived", "Students stopped using software"), "The five test laptops became faster after repair, which supports repairing the others first.", correct_position=2)),
            _nested("reading_comprehension", title="A revised workshop", text="Feedback showed that participants understood the demonstrations but needed more time to practise. The organizer therefore shortened the opening talk and added a longer guided task.", question=_choice("Which evidence caused the workshop to change?", "Participants needed more practice time", ("The venue became unavailable", "The demonstrations were removed"), "The feedback specifically says participants needed more time to practise.", correct_position=1)),
        ],
    ),
    "listening": _skill_lesson(
        ...
        [
            ...
            _nested("listening_comprehension", title="A project update", text="The design is ready and the client has approved it. However, the testing stage will start two days late because one device has not arrived. In conclusion, the final deadline is still realistic.", question=_choice("What problem does the speaker introduce after ‘however’?", "Testing will start two days late", ("The client rejected the design", "The final deadline was cancelled"), "After however, the speaker says testing will start two days late.", correct_position=1)),
            _nested("listening_comprehension", title="Choosing a training format", text="Online training is flexible. For example, staff can complete each section when their schedule is quiet. The company will therefore offer the course online next month.", question=_choice("What example of flexibility does the speaker give?", "Staff can study when their schedule is quiet", ("The course requires a fixed classroom", "The company cancelled the course"), "The speaker’s example is that staff can study when their schedule is quiet.", correct_position=0)),
            _nested("listening_comprehension", title="Revising a delivery plan", text="First, the supplier confirmed that the materials are ready. On the other hand, the larger vehicle is unavailable until Friday. As a result, the team will make two smaller deliveries.", question=_choice("What decision follows from the vehicle problem?", "Make two smaller deliveries", ("Cancel the materials", "Wait for a new supplier"), "As a result introduces the decision to make two smaller deliveries.", correct_position=2)),
        ],
    ),
    "speaking": _skill_lesson(
    ...
    "pronunciation": _skill_lesson(
        ...
        [
            {"type": "pronunciation_drill", "data": {"sound_label": "Aspirated P", "ipa": "/p/", "instructions": "Close both lips, build pressure, and release /p/ with a clear puff of air at the start of a stressed word. Keep the throat quiet.", "tips": ["Test the air with a small piece of paper.", "Do not voice the sound before the lips open."], "practice_items": ["project plan", "payment problem", "Please print the proposal."], "native_hint": None}},
            ...
            {"type": "pronunciation_drill", "data": {"sound_label": "P and B across a thought group", "ipa": "/p/", "instructions": "Keep /p/ airy and /b/ voiced while placing the strongest stress on the important message word.", "tips": ["Mark the stressed word before speaking.", "Keep the lips active even in the middle of the phrase."], "practice_items": ["Please bring the project brief.", "The backup plan is practical.", "We prepared a better proposal."], "native_hint": None}},
...
_CURATED_LESSONS["B2"] = {
    ...
    "reading": _skill_lesson(
        ...
        [
            ...
            _nested("reading_comprehension", title="Four-day trial", text="A six-week trial found that employees on a four-day schedule reported higher concentration and completed the same number of tasks. However, the trial involved only one department during a quiet season. The results support a longer test, not an immediate company-wide change.", question=_choice("Which limitation reduces the scope of the trial’s conclusion?", "It covered one department during a quiet season", ("Employees completed the same number of tasks", "The company has already changed every schedule"), "The stated limitation is that the trial covered one department during a quiet season.", correct_position=1)),
            _nested("reading_comprehension", title="AI support tool", text="A support team used an AI tool to draft routine replies. Average response time fell by 18 percent. Human review was still part of the process: supervisors checked every message before it was sent. The author therefore does not claim that the tool can work safely on its own.", question=_choice("What qualification does the author make?", "Human review was still part of the process", ("Response time increased by 18 percent", "The tool independently sent every reply"), "Human review was still part of the process because supervisors checked every message before it was sent.", correct_position=2)),
            _nested("reading_comprehension", title="Transport survey", text="A neighborhood survey found strong support for a later evening bus. Yet most responses came from commuters who already use the route. The findings justify a broader consultation, but they do not represent every resident.", question=_choice("Why should the survey conclusion remain limited?", "Most responses came from existing commuters", ("The route has already closed", "Every resident answered the survey"), "Most responses came from existing commuters, so the sample cannot represent every resident.", correct_position=0)),
        ],
    ),
    "listening": _skill_lesson(
        ...
        [
            ...
            _nested("listening_comprehension", title="Remote onboarding", text="Remote onboarding appears to reduce travel costs and gives new staff more flexibility. Having said that, the first week probably works better when each employee has a named colleague to contact.", question=_choice("What does the speaker recommend for the first week?", "Give each employee a named contact", ("Require every employee to travel", "Remove all onboarding support"), "After the qualification, the speaker recommends giving each employee a named contact.", correct_position=0)),
            _nested("listening_comprehension", title="A cautious forecast", text="Sales are likely to improve next quarter, mainly because two delayed products will finally launch. Nevertheless, the estimate depends on shipping costs remaining stable.", question=_choice("What condition could affect the forecast?", "Whether shipping costs remain stable", ("Whether both products stay delayed forever", "Whether the company stops tracking sales"), "The speaker says the estimate depends on shipping costs remaining stable.", correct_position=2)),
            _nested("listening_comprehension", title="Qualified recommendation", text="The pilot seems to have reduced routine errors. Even so, the sample was relatively small, and the team should probably repeat the test before adopting the process everywhere.", question=_choice("What does the speaker recommend before wider adoption?", "Repeat the test", ("Ignore the pilot", "Adopt it everywhere immediately"), "The speaker recommends repeating the test because the sample was small.", correct_position=1)),
        ],
    ),
    "speaking": _skill_lesson(
    ...
    "pronunciation": _skill_lesson(
        "Prominence and reduction",
        "Use sentence stress to highlight contrast while reducing less important words.",
        "Mark the information focus, then coordinate pitch movement with unstressed reductions.",
        [
            {"type": "pronunciation_drill", "data": {"sound_label": "Contrastive prominence", "ipa": "/ˈ/", "instructions": "On the corrected word, lengthen the stressed syllable slightly and make a clear pitch movement. Keep surrounding words lighter.", "tips": ["Decide which information is new or corrected.", "Avoid stressing every word equally."], "practice_items": ["I said TUESday, not THURSday.", "We need the FINAL draft, not the FIRST one.", "She ordered FIFteen, not FIFty."], "native_hint": None}},
            {"type": "pronunciation_drill", "data": {"sound_label": "Weak function words", "ipa": "/ə/", "instructions": "Shorten unstressed function words and use a relaxed central vowel while keeping content words clear.", "tips": ["Keep the jaw and tongue relaxed on the weak syllable.", "Do not delete the word completely."], "practice_items": ["We could have asked them earlier.", "Send it to the manager.", "A report for the client."], "native_hint": None}},
            {"type": "pronunciation_drill", "data": {"sound_label": "Thought-group contrast", "ipa": "/ˈ/", "instructions": "Pause lightly between thought groups and use pitch and length to highlight the contrasting word in each group.", "tips": ["Underline one focus word per thought group.", "Reduce the surrounding grammar words instead of stressing everything."], "practice_items": ["The FIRST option is cheaper, but the SECOND is safer.", "We could approve it NOW, or review it AGAIN tomorrow.", "The DATA looks promising, although the SAMPLE is small."], "native_hint": None}},
            {"type": "multiple_choice", "data": _choice("In ‘I said TUESday, not THURSday,’ which word should carry the first contrastive prominence?", "TUESday", ("said", "not"), "TUESday carries the first contrastive prominence because it is the corrected day.", correct_position=2)},
        ],
    ),
    "discourse": _skill_lesson(
