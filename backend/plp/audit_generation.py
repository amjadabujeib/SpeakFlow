...
def audit_generation(*, domain: str, level: str) -> dict:
    ...
    try:
        ...
        return {
            ...
            "activities": [
                _activity_summary(item)
                for item in generated["content"]["activities"]
            ],
        }
    finally:
        retriever.close()
...
def main() -> None:
    ...
    parser.add_argument(
        "--level", default="B1", choices=("A1", "A2", "B1", "B2")
    )
    args = parser.parse_args()
    try:
        print(json.dumps(audit_generation(domain=args.domain, level=args.level), indent=2))
    finally:
        get_engine().dispose()
