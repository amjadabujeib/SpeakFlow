...
def _validate_manifest(
    ...
    if not isinstance(file_records, dict) or set(file_records) != set(payloads):
        raise CurriculumSnapshotError(
            "curriculum snapshot file manifest is incomplete"
        )
...
def read_snapshot(path: Path = SNAPSHOT_PATH) -> CurriculumSnapshot:
    ...
    source_ids = {
        _required_string(row, "id", context="curriculum source")
        for row in sources
    }
    ...
    for row in sources:
        ...
        if len(str(row["checksum"])) != 64:
            raise CurriculumSnapshotError(
                f"{source_id} has an invalid source checksum"
            )
    ...
    for row in concepts:
        concept_id = _required_string(
            row, "id", context="curriculum concept"
        )
        source_id = _required_string(
            row, "source_id", context=concept_id
        )
        external_id = _required_string(
            row, "external_id", context=concept_id
        )
        content_hash = _required_string(
            row, "content_hash", context=concept_id
        )
        for key in (
            "concept_type",
            "title",
        ...
        if row.get("cefr_level") is not None and not isinstance(
            ...
            raise CurriculumSnapshotError(
                f"{concept_id} has an invalid CEFR level"
            )
        if row.get("description") is not None and not isinstance(
            row.get("description"), str
        ):
            raise CurriculumSnapshotError(
                f"{concept_id} has an invalid description"
            )
        ...
        if source_id not in source_ids:
            raise CurriculumSnapshotError(
                f"{concept_id} references an unknown source"
            )
        if (
    ...
    if embedding_rows != set(range(embeddings.shape[0])):
        raise CurriculumSnapshotError(
            "curriculum snapshot has unreferenced embeddings"
        )
...
def _upsert_snapshot(session: Session, snapshot: CurriculumSnapshot) -> dict[str, Any]:
    ...
    concept_source_ids = {
        str(record["source_id"]) for record in snapshot.concepts
    }
    existing = {
        item.id: item
