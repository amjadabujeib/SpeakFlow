...
void registerLearningPlanActiveTests() {
  testWidgets('lesson launched from Home covers all shell chrome', (
    tester,
  ) async {
    ...
    final lesson = document!.lessons.firstWhere(
      (candidate) =>
          ...
          document.statusOf(candidate) != PlpLessonStatus.locked,
    );
    ...
    await tester.ensureVisible(find.text(lesson.title).first);
    ...
    expect(bounds.left, closeTo(18, 0.5));
    expect(393 - bounds.right, closeTo(18, 0.5));
    expect(tester.takeException(), isNull);
  });

  testWidgets('lesson overview continue button uses symmetric phone insets', (
    tester,
  ) async {
    tester.view.physicalSize = const Size(393, 852);
    tester.view.devicePixelRatio = 1;
  ...
  testWidgets('fill-blank question stays readable above a tall keyboard', (
    tester,
  ) async {
    tester.view.physicalSize = const Size(393, 852);
    tester.view.devicePixelRatio = 1;
    ...
    final source = document!.lessons.first;
    final lesson = PlpLesson(
      ...
      requiredLessonIds: source.requiredLessonIds,
      personalizationReason: source.personalizationReason,
      lessonRole: source.lessonRole,
      canDoStatement: source.canDoStatement,
      contentInstanceId: source.contentInstanceId,
      grounding: source.grounding,
      contentStatus: PlpContentStatus.ready,
      content: PlpLessonContent(
        intro: source.content!.intro,
        activities: [
          PlpActivity.fromJson({
            'id': 'keyboard-fill-blank',
            'type': 'fill_blank',
            'phase': 'guided_practice',
            'prompt': 'The device needs a software _____ before it restarts.',
            'accepted_answers': ['update'],
            'explanation': 'Update completes the device instruction.',
          }, 'keyboard lesson'),
        ],
      ),
    );
    ...
    await tester.showKeyboard(
      find.byKey(const ValueKey('lesson-fill-blank-answer')),
    );
    await tester.pumpAndSettle();

    final prompt = tester.getRect(
      find.byKey(const ValueKey('lesson-fill-blank-prompt')),
    );
    final answer = tester.getRect(
      find.byKey(const ValueKey('lesson-fill-blank-answer')),
    );
    expect(prompt.top, greaterThanOrEqualTo(0));
    expect(prompt.bottom, lessThan(answer.top));
    expect(answer.bottom, lessThan(852 - 360));
  ...
  testWidgets('finished first-week generation opens the active plan', (
    ...
    await tester.pumpWidget(
      testApp(home: LearningPlanScreen(repository: repository)),
    );
    await tester.pump();
  ...
  testWidgets('settings-tracked regeneration overlays the existing plan', (
    tester,
  ) async {
    final document = await tester.runAsync(
      () => const AssetPlpRepository().loadPlan(),
    );
    final appState = AppState()..trackPlanGeneration('manual-generation-job');
    ...
    await tester.pumpWidget(
      ProviderScope(
        ...
        child: MaterialApp(
          home: LearningPlanScreen(
            repository: _ActiveGenerationRepository(document!),
          ),
        ),
      ),
    );
