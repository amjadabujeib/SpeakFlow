...
void main() {
  ...
  test('pronunciation progress can complete without claiming mastery', () {
    final result = PlpAttemptResult.fromJson({
      'correct': null,
      'score': 0,
      'diagnostic_score': 76,
      'explanation': 'Continue without a mastery record.',
      'first_attempt': false,
      'mastery_evidence_recorded': false,
      'pronunciation_target_completed': true,
      'pronunciation_mastery_verified': null,
      'lesson_completed': false,
      'lesson_progress': {
    ...
    expect(result.pronunciationTargetCompleted, isTrue);
    expect(result.pronunciationMasteryVerified, isNull);
    expect(result.masteryEvidenceRecorded, isFalse);
    expect(result.score, 0);
    expect(result.diagnosticScore, 76);
    expect(
      result.pronunciationActivityProgress['sound_check']!.unverifiedTargetKeys,
      {'bag'},
    );
  });
