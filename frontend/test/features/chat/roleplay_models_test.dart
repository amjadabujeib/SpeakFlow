...
import 'package:speakflow/features/chat/roleplay_text_policy.dart';

void main() {
  test('typed roleplay rejects Arabic before opening a provider turn', () {
    expect(roleplayTypedTurnError('ابتثجحخ'), roleplayEnglishTurnMessage);
    expect(roleplayTypedTurnError('123 🎭'), roleplayEnglishTurnMessage);
    expect(roleplayTypedTurnError('I need help, please.'), isNull);
  });
  ...
  test('short session keeps independent provisional category scores', () {
    ...
    expect(feedback.scores['interaction'], isNull);
    expect(feedback.scores.containsKey('overall'), isFalse);
