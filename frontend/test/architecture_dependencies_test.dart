...
void main() {
  test('API configuration owns the versioned HTTP and WebSocket paths', () {
    expect(ApiConfig.uri('/auth/me').path, '/api/v1/auth/me');
    expect(ApiConfig.websocketUri('/chat/ws').path, '/api/v1/chat/ws');
  });
  ...
  test('feature dependencies can be replaced through Riverpod', () async {
    ...
    expect(requestedUri.path, '/api/v1/learning-plan/generations');
  });

  test(
    'legacy PLP repository delegates to the versioned feature API',
    () async {
      late Uri requestedUri;
      ...
      expect(requestedUri.toString(), 'https://example.test/api/v1/me/profile');
    },
  );
